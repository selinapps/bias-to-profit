-- Performance optimization migration for trades tracking
-- This migration adds comprehensive indexes and optimizations for better performance
...
-- Create trigger to auto-refresh materialized view when trades are updated
CREATE OR REPLACE FUNCTION public.trigger_refresh_daily_performance()
RETURNS TRIGGER AS $$
BEGIN
  -- Refresh materialized view asynchronously
  PERFORM pg_notify('refresh_daily_performance', '');
  RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

-- Create trigger for trades table
DROP TRIGGER IF EXISTS trigger_trades_daily_performance ON public.trades;
CREATE TRIGGER trigger_trades_daily_performance
  AFTER INSERT OR UPDATE OR DELETE ON public.trades
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_refresh_daily_performance();
