...
DROP POLICY IF EXISTS "Bias state select" ON public.bias_state;
DROP POLICY IF EXISTS "Bias state insert" ON public.bias_state;
DROP POLICY IF EXISTS "Bias state update" ON public.bias_state;
DROP POLICY IF EXISTS "Bias state delete" ON public.bias_state;

CREATE POLICY "Bias state select"
  ON public.bias_state
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Bias state insert"
  ON public.bias_state
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "Bias state update"
  ON public.bias_state
  FOR UPDATE
  TO authenticated
  USING (true)
...
CREATE POLICY "Bias state delete"
  ON public.bias_state
  FOR DELETE
  TO authenticated
  USING (true);
