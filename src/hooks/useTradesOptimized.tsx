...
interface UseTradesOptimizedReturn {
  ...
  getDailyPerformanceMetrics: () => Promise<any>;
}
...
export function useTradesOptimized(): UseTradesOptimizedReturn {
  ...
  const handleRealtimeUpdate = useCallback((payload: any) => {
    ...
    debounceTimeoutRef.current = setTimeout(() => {
      console.log('Fetching trades after realtime update...');
  ...
  const addTrade = useCallback(async (tradeData: Omit<TradeInsert, 'user_id'>) => {
    ...
    // Optimistic update: Add the new trade to the state immediately
    if (data && data.length > 0) {
      const newTrade = data[0] as Trade;
      setTrades(prev => [newTrade, ...prev]); // Add to beginning of list
    ...
    // Also force refresh to ensure consistency (but optimistic update shows immediately)
  ...
  const closeTrade = useCallback(async (id: string, exitPrice: number, exitTime?: Date, additionalProfit = 0) => {
    ...
    // Optimistic update - immediately move trade to closed status
    ...
    // Clear cache and force refresh to ensure consistency
    if (user) {
      tradesCache.delete(user.id);
      // Force a refresh after a short delay to ensure the database update is reflected
      setTimeout(() => {
        fetchTrades(true);
      }, 100);
    }
