...
export function useBiasState(targetDate?: Date) {
  const { user } = useAuth();
  ...
  const dayKey = useMemo(() => format(targetDate || new Date(), 'yyyy-MM-dd'), [targetDate]);
  ...
  const loadBiasFromLocal = useCallback((): BiasStateResponse => {
    ...
    try {
      ...
      logger.error('Failed to load bias state from local storage', error);
      return null;
    }
  }, [dayKey, user?.id]);
  ...
  const persistBiasToLocal = useCallback(
    (snapshot: BiasStateSnapshot) => {
      ...
      try {
        ...
        logger.error('Failed to persist bias state locally', error);
      }
    },
    [dayKey, user?.id]
  );
  ...
  const clearLocalBias = useCallback(() => {
    ...
    try {
      ...
      logger.error('Failed to clear local bias cache', error);
    }
  }, [dayKey, user?.id]);
  ...
  const fetchFromTable = useCallback(async (): Promise<BiasStateResponse> => {
    ...
    if (error) {
      if (isMissingRelationError(error)) {
        logger.error(biasStateMissingSchemaMessage, error);
  ...
  const fetchCurrent = useCallback(async () => {
    ...
    if (shouldAttemptRpc) {
      ...
      if (error) {
        if (isMissingFunctionError(error)) {
          ...
          logger.error('Error loading bias state', error);
          setBiasState(null);
          setLoading(false);
          return;
        }
    ...
    try {
      ...
      logger.error('Error loading bias state fallback', fallbackError);
      setBiasState(null);
    } finally {
      setLoading(false);
    }
    ...
    async (result: BiasQuizResult) => {
      ...
      try {
        ...
        if (error) {
          ...
          // Handle unique constraint violation by falling back to direct table mutation
          if (error.code === '23505' && error.message.includes('duplicate key value violates unique constraint')) {
            logger.warn(
              'Unique constraint violation detected. Falling back to direct table mutation to handle existing bias state.'
            );
