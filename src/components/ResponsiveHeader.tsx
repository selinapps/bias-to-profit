            ...
            <div className="mt-1 p-2 bg-secondary/30 rounded-lg border border-trading-border/50">
              <div className="flex items-start gap-2">
                <div className="flex-shrink-0 mt-0.5">
                  <div className={`w-2 h-2 rounded-full ${currentSession.color && currentSession.color.includes('red') ? 'bg-red-400' : currentSession.color && currentSession.color.includes('blue') ? 'bg-blue-400' : currentSession.color && currentSession.color.includes('emerald') ? 'bg-emerald-400' : currentSession.color && currentSession.color.includes('purple') ? 'bg-purple-400' : currentSession.color && currentSession.color.includes('yellow') ? 'bg-yellow-400' : 'bg-green-400'}`}></div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-foreground">{currentSession.name}</p>
                  <p className="text-xs text-trading-accent font-medium mt-0.5">{currentSession.priceHint}</p>
                  <p className="text-xs text-trading-muted mt-0.5">{currentSession.description}</p>
                  <p className="text-xs text-trading-muted mt-1">
                    {currentSession.localTime} | Local: {currentTime.toLocaleTimeString('en-US', { 
                      hour: '2-digit', 
                      minute: '2-digit',
                      hour12: false 
                    })}
        ...
        {showMobileMenu && (
          <Card className="mx-4 mb-4 border-trading-border bg-background/95 backdrop-blur-md">
            <CardContent className="p-4">
              {/* Journal Mode Toggle - Mobile */}
              <div className="mb-3 pb-3 border-b border-trading-border">
                <p className="text-xs text-trading-muted mb-2">Journal Mode</p>
                <div className="flex gap-2">
                  <Button
                    variant={settings.journalMode === 'advanced' ? 'default' : 'outline'}
                    size="sm"
                  ...
                  <Button
                    variant={settings.journalMode === 'simple' ? 'default' : 'outline'}
                    size="sm"
        ...
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-secondary/30 rounded-lg p-1">
              <Button
                variant={settings.journalMode === 'advanced' ? 'default' : 'ghost'}
                size="sm"
                onClick={() => updateSetting('journalMode', 'advanced')}
                className="h-8 px-3"
              >
              ...
              <Button
                variant={settings.journalMode === 'simple' ? 'default' : 'ghost'}
                size="sm"
                onClick={() => updateSetting('journalMode', 'simple')}
                className="h-8 px-3"
              >
          ...
          <div className="px-6 pb-3">
            <div className="flex items-start gap-3 p-3 bg-secondary/20 rounded-lg border border-trading-border/50">
              <div className="flex-shrink-0 mt-1">
                <div className={`w-3 h-3 rounded-full ${currentSession.color && currentSession.color.includes('red') ? 'bg-red-400' : currentSession.color && currentSession.color.includes('blue') ? 'bg-blue-400' : currentSession.color && currentSession.color.includes('emerald') ? 'bg-emerald-400' : currentSession.color && currentSession.color.includes('purple') ? 'bg-purple-400' : currentSession.color && currentSession.color.includes('yellow') ? 'bg-yellow-400' : 'bg-green-400'}`}></div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-trading-muted">| Local: {currentTime.toLocaleTimeString('en-US', { 
                    hour: '2-digit', 
                    minute: '2-digit',
                    hour12: false 
                  })}</span>
                </div>
                <p className="text-sm font-medium text-trading-accent mb-1">{currentSession.priceHint}</p>
