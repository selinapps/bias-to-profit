...
export function DailyWrap() {
  ...
  const worstHour = Object.entries(hourPerformance)
    .filter(([, data]) => data.pnl < 0) // Only consider hours with losses
    .reduce<{ hour: number; data: { trades: number; pnl: number } } | null>((worst, [hour, data]) => {
