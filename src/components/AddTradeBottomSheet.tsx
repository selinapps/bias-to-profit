...
import { AlertTriangle, Camera, TrendingUp, TrendingDown, Lock, Zap, Shield } from 'lucide-react';
...
export function AddTradeBottomSheet({ isOpen, onClose, biasState, onRequestBiasEdit, refreshTrades }: AddTradeBottomSheetProps) {
  const { user } = useAuth();
  ...
  const allowedModels = useMemo(() => {
    if (!biasState || biasState.bias === 'FLAT' || !biasState.market_state) {
  ...
  const canUseExecutionModel = Boolean(biasState && biasState.bias !== 'FLAT' && biasState.market_state);
  const baseFormValid = Boolean(entryPrice && stopLoss && asset && locations.length > 0 && aggression.length > 0 && selectedChallengeId);
  const challengeValid = !validationError && !isLocked && isRiskAllowed;
  const canConfirmEntry = baseFormValid && canUseExecutionModel && Boolean(model) && checklistComplete && challengeValid;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const biasSnapshot = {
        ...
        // Map execution model to database model
        const dbModel = model.startsWith('TREND') ? 'trend' : 'mean_reversion';
            ...
            <Card className="bg-muted/10 border-trading-border">
              <div className="p-4 space-y-3">
                ...
                <div className="space-y-2">
                  <Label htmlFor="entry-time" className="text-xs text-trading-muted">Adjust entry time</Label>
                  <Input
                    id="entry-time"
                    type="datetime-local"
                    value={entryTime.toISOString().slice(0, 16)}
                    onChange={(e) => setEntryTime(new Date(e.target.value))}
                    className="h-8 text-sm bg-input border-trading-border"
                  />
                  <Button
                    type="button"
                    variant="outline"
          ...
          {activeChallenge && (
            <div>
              ...
              <Select value={selectedChallengeId} onValueChange={setSelectedChallengeId}>
                ...
                <SelectContent>
                  <SelectItem value={activeChallenge.id}>
        ...
        <div className="sticky bottom-0 bg-trading-card border-t border-trading-border p-4">
          ...
          {!canConfirmEntry && (
            <div className="mt-2 text-xs text-muted-foreground text-center">
              {!baseFormValid && 'Fill required fields • '}
              {!canUseExecutionModel && 'Set bias/state • '}
              {!model && 'Select execution model • '}
